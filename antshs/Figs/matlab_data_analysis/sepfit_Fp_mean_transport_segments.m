function [estlambda, estFp, wlsval, estmeanfield, meandata, t] = sepfit_Fp_mean_transport_segments
% SEPFIT_FP_MEAN_TRANSPORT_SEGMENTS   Fit constant Fp to means of Aurelie's
%   data, where rates are fitted first.

% [estlambda, estFp, wlsval, estmeanfield, meandata, t] = sepfit_Fp_mean_transport_segments;
%
...
[~, estmeanfield] = ode45( @(t,meanstate)(estauglaplacian*meanstate ...
                                    + [ zeros(length(estlambda)+1,1) ; estFp*(meanstate(1))/mL ] ...
                                    - [ zeros(length(estlambda)+1,1) ; d*mu*max(0, g - (Fl/mL)*(meanstate(2)) ) ] ), ...
                                    simtime, meandata(:,1) );
...
function lssum = clsmetric( params, t, meandata, Fl, mL, mu, g, lambda )
    ...
    [~, meanfield] = ode15s( @(t,meanstate)(auglaplacian*meanstate ...
                                        + [ zeros(length(lambda)+1,1) ; Fp*(meanstate(1))/mL ] ...
                                        - [ zeros(length(lambda)+1,1) ; d*mu*max(0, g - (Fl/mL)*(meanstate(2)) ) ] ), ...
                                        t, meandata(:,1) );
    meanfield = meanfield';
