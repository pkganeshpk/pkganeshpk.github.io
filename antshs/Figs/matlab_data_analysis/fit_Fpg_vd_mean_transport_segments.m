function [estlambda, estFpg, estvd, wlsval, estmeanfield, meandata, t] = fit_Fpg_vd_mean_transport_segments
% FIT_FPG_VD_MEAN_TRANSPORT_SEGMENTS   Fit (Fpg, vd) to means of Aurelie's data.

% [estlambda, estFpg, estvd, wlsval, estmeanfield, meandata, t] = fit_Fpg_vd_mean_transport_segments;
%
...
num_cts = 2;
num_params = num_cts + num_rates;

[estparams, fval, exitflag] = ...
    fmincon( @(params)( lsmetric(params, t, meandata, Fl, mL, mu, g) ), ...
            0.5*randperm(num_params)/num_params, ...
            [], [], ... % inequalities (A*x <= b)
            [], [], ... % equalities (Aeq*x == beq)
            [ zeros(num_rates,1); 1e-6; 1e-6 ], ... % lower bounds
            [ (1/(2*dT))*ones(num_rates,1); 0.5; 1 ], ... % upper bounds
...
function lssum = lsmetric( params, t, meandata, Fl, mL, mu, g )
    % This maps the number of elements in lambdavec to the number of states
    num_cts = 2;
    num_dstates = max( roots( [1 -1 -(length(params) - num_cts)] ) );
    Fpg = params(end-1);
    vd = params(end);
    ...
    lambda(offdiagonals) = params(1:(end-num_cts));
