% CL_ANT_SHS_ABM   Closed-loop ant stochastic hybrid system, approximate agent-based model
%
...
%    stateperm = randperm(length(lambda));
    ...
    for i = 2:length(time)
        ...
        Fg = (num_in_state{stateind('pulling-right')}(mcrun, i-1) - num_in_state{stateind('pulling-left')}(mcrun, i-1))*Fpg*(vd - v(mcrun, i-1));
...
vdot = (Fpg*vd/mL) * pullvec;
auglaplacian = [ laplacian zeros(length(lambda),2) ;
                 zeros(1,length(lambda)) 0 1 ;
                 vdot 0 0 ];
[t, mean_field_counts] = ode45( @(t,meanstate)(auglaplacian*meanstate - [ zeros(length(lambda)+1,1) ; Fpg*meanstate(end)*(meanstate(stateind('pulling-right')) - meanstate(stateind('pulling-left')))/mL ] ), time, initial_mean_state );
